...
function resetProductNameComboboxState(root) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    items[i].classList.remove('combobox-item--multiline');
  }
}
...
function updateProductNameComboboxMenuWidth(root) {
  ...
  content.style.width = Math.round(root.getBoundingClientRect().width) + 'px';
...
function updateProductNameComboboxMenuPosition(root) {
  ...
  syncProductNameComboboxItemLayouts(root);
}

/** @param {Element} root */
function syncProductNameComboboxItemLayouts(root) {
  var content = getProductNameComboboxContent(root);
  if (!content) return;
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    item.classList.remove('combobox-item--multiline');
    if (!textEl || item.hidden) continue;
    textEl.style.whiteSpace = 'nowrap';
    var needWrap = textEl.scrollWidth > textEl.clientWidth;
    textEl.style.whiteSpace = '';
    if (needWrap) {
      item.classList.add('combobox-item--multiline');
    }
  }
}
...
function initClientSelectMenu() {
  ...
  function selectClient(label) {
    ...
    resetClientNameLineBreak(label);
    closeMenu();
    if (searchInput) {
      searchInput.value = '';
      filterClientMenu('');
...
var clientNameLineBreakState = {
  base: '',
  lastValid: '',
};

function stripClientNameLineBreaks(value) {
  return String(value || '').replace(/\r\n|\n|\r/g, '');
}

function isValidClientNameLineBreakValue(value, base) {
  return stripClientNameLineBreaks(value) === String(base || '');
}

function updateClientNameLineBreakValidation() {
  var textarea = document.getElementById('client-name-line-break-input');
  var errorEl = document.getElementById('client-name-line-break-error');
  var wrap = document.getElementById('client-name-line-break-text-area-wrap');
  var panel = document.getElementById('client-name-line-break-panel');
  var hiddenName = document.querySelector('input[name="client.clientName"]');
  if (!textarea || !errorEl) return true;

  var base = hiddenName ? hiddenName.value : '';
  var panelOpen = panel && !panel.hidden;
  var isValid =
    !panelOpen || !base || isValidClientNameLineBreakValue(textarea.value, base);

  if (!isValid) {
    errorEl.hidden = false;
    textarea.setAttribute('aria-invalid', 'true');
    textarea.setAttribute('aria-describedby', 'client-name-line-break-error');
    if (wrap) wrap.classList.add('client-name-line-break-panel__text-area-wrap--error');
  } else {
    errorEl.hidden = true;
    textarea.removeAttribute('aria-invalid');
    textarea.removeAttribute('aria-describedby');
    if (wrap) wrap.classList.remove('client-name-line-break-panel__text-area-wrap--error');
  }
  return isValid;
}
 ...
 * @returns {string | null}
 */
function predictTextareaInputValue(textarea, inputType, data) {
  ...
  if (inputType === 'deleteContentBackward') {
    if (start !== end) return val.slice(0, start) + val.slice(end);
    if (start <= 0) return val;
    return val.slice(0, start - 1) + val.slice(start);
  }
  if (inputType === 'deleteContentForward') {
    if (start !== end) return val.slice(0, start) + val.slice(end);
    if (end >= val.length) return val;
...
function resetClientNameLineBreak(clientLabel) {
  var textarea = document.getElementById('client-name-line-break-input');
  var panel = document.getElementById('client-name-line-break-panel');
  var openBtn = document.getElementById('client-name-line-break-open');
  var label = clientLabel || '';
  clientNameLineBreakState.base = label;
  clientNameLineBreakState.lastValid = label;
  if (textarea) textarea.value = label;
  if (panel) panel.hidden = true;
  if (openBtn) openBtn.setAttribute('aria-expanded', 'false');
  updateClientNameLineBreakValidation();
}

function initClientNameLineBreakPanel() {
  var openBtn = document.getElementById('client-name-line-break-open');
  var closeBtn = document.getElementById('client-name-line-break-close');
  var panel = document.getElementById('client-name-line-break-panel');
  var textarea = document.getElementById('client-name-line-break-input');
  var hiddenName = document.querySelector('input[name="client.clientName"]');
  if (!openBtn || !panel || !textarea) return;

  function getClientLabel() {
    return hiddenName ? hiddenName.value : '';
  }

  function syncLineBreakBaseFromClient() {
    var base = getClientLabel();
    clientNameLineBreakState.base = base;
    if (!isValidClientNameLineBreakValue(textarea.value, base)) {
      textarea.value = base;
    }
    clientNameLineBreakState.lastValid = textarea.value;
  }
  ...
  function setPanelOpen(open) {
    ...
    if (open) {
      ...
      var base = getClientLabel();
      if (base && !isValidClientNameLineBreakValue(textarea.value, base)) {
        textarea.value = base;
        clientNameLineBreakState.lastValid = base;
        syncInvoicePreview();
      }
      updateClientNameLineBreakValidation();
    }
  ...
  textarea.addEventListener('beforeinput', function (e) {
    var base = clientNameLineBreakState.base;
    if (!base) {
      e.preventDefault();
      return;
    }
    var predicted = predictTextareaInputValue(textarea, e.inputType, e.data);
    if (predicted === null) return;
    if (!isValidClientNameLineBreakValue(predicted, base)) {
      e.preventDefault();
    }
  });
  ...
  textarea.addEventListener('input', function () {
    ...
    textarea.value = clientNameLineBreakState.lastValid;
    updateClientNameLineBreakValidation();
  });
...
function syncInvoicePreview() {
  ...
  var lineBreakTa = document.getElementById('client-name-line-break-input');
  var cnRaw = cnIn ? cnIn.value : '';
  updateClientNameLineBreakValidation();
  var cnDisplay = cnRaw;
  if (lineBreakTa && isValidClientNameLineBreakValue(lineBreakTa.value, cnRaw)) {
    cnDisplay = lineBreakTa.value;
  }
  var honIn = form.querySelector('input[name="client.honorific"]');
  var recClient = splitClientForPreview(cnDisplay, honIn ? honIn.value : '');
